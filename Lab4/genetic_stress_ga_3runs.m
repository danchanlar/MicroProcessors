...
function genetic_stress_ga_3runs(filename)

    N = 30; Gmax = 100; mutation_rate = 0.05;

    % Τρέξε τον ΓΑ 3 φορές
    [scoreG1, bestW1, bestS1] = run_ga_once(filename, N, Gmax, mutation_rate);
    [scoreG2, bestW2, bestS2] = run_ga_once(filename, N, Gmax, mutation_rate);
    [scoreG3, bestW3, bestS3] = run_ga_once(filename, N, Gmax, mutation_rate);

    figure;
    plot(1:Gmax, scoreG1,'-o', 1:Gmax, scoreG2,'-s', 1:Gmax, scoreG3,'-^');
    xlabel('generation g');
    ylabel('number of switches (max per generation)');
    title(sprintf('GA Evolution (3 runs)  L=2, N=%d, m=%.2f, G=%d', N, mutation_rate, Gmax));
    legend('Run 1','Run 2','Run 3','Location','best');
    grid on;
    print('-dpng','ga_evolution_3runs.png');

    gaSaveBestWorkload(bestW1, bestS1, 'ga_best_workload_run1.txt');
    gaSaveBestWorkload(bestW2, bestS2, 'ga_best_workload_run2.txt');
    gaSaveBestWorkload(bestW3, bestS3, 'ga_best_workload_run3.txt');

    fprintf('Run1 best: %d  | Run2 best: %d  | Run3 best: %d\n', bestS1, bestS2, bestS3);
end

function [scoreG, best_global_workload, best_global_score] = run_ga_once(filename, N, Gmax, mutation_rate)
    ...
    for g = 1:Gmax
        ...
        scores = zeros(1, N);
        for p = 1:N
            scores(p) = evaluateWorkload(circuit0, population{p});
        end

        % Κορυφαίος της γενιάς
        ...
        % Επιλογή 2 γονέων (top-2, διαφορετικοί)
        ...
        for idx = 2:2:N
            ...
            if idx+1 <= N, new_population{idx+1} = c2; end
        end
